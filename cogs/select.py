...
class SelectCog(commands.Cog):
    ...
    @app_commands.command(name="select", description="aa")
    ...
    async def select(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            for number in range(1, 10):
                role = discord.utils.get(interaction.guild.roles, name=f"test-{number}")
                if role:
                    role_count += 1
                    embed.add_field(name=f"", value=f"{role.mention}", inline=False)

            role1 = discord.utils.get(interaction.guild.roles, name=f"test-1")
            if role1:
            ...
            role2 = discord.utils.get(interaction.guild.roles, name=f"test-2")
            if role2:
            ...
            role3 = discord.utils.get(interaction.guild.roles, name=f"test-3")
            if role3:
            ...
            role4 = discord.utils.get(interaction.guild.roles, name=f"test-4")
            if role4:
            ...
            role5 = discord.utils.get(interaction.guild.roles, name=f"test-5")
            if role5:
            ...
            if role_count == 0:
                embed.add_field(name=f"Brak ról", value=f"", inline=False)
            else:
    ...
    async def __select_callback(interaction: discord.Interaction):
        try:
            ...
            for number in range(1, 10):
                ...
                embed: Embed = discord.Embed(title=f"", description=f"usunieto", color=config_file['EMBED_COLOR'])
            if interaction.data['custom_id'] != "delete":
                role = discord.utils.get(interaction.guild.roles, name=f"test-{interaction.data['custom_id']}")
                embed: Embed = discord.Embed(title=f"", description=f"wybrano {interaction.data['custom_id']} - {role.mention}",
